...
export function createCrudStore<iDB extends { id: string }>
  (apiUrl: string, modelXX: any, createModel: Function, seconds: number, etc?: any,) {
  ...
  return defineStore(apiUrl, (): iModels<iDB, iModel> => {
    ...
    const list = computed(() => _refs.value.filter(x => _outputs.value.includes(x?.id))?.map((x: Reactive<any>) => createModel(x)) ?? []);
